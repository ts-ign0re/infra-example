...
- Локальный Kubernetes кластер (Docker Desktop Kubernetes, kind, k3d, или minikube)
- Docker (для образов и k8s)

### Arch Linux
- **[Специальная инструкция для Linux](docs/03-LINUX-SETUP.md)** - Arch, Ubuntu, Debian - установка k3d/minikube + настройка кластера

## 📚 Документация

### Установка и настройка
1. **[Linux Setup](docs/03-LINUX-SETUP.md)** - Для Arch, Ubuntu, Debian (k3d/minikube)
2. **[Настройка инфраструктуры](docs/01-SETUP-INFRASTRUCTURE.md)** - Поднимаем Tilt + Kubernetes
3. **[Быстрый старт](docs/02-QUICKSTART.md)** - Подключение к сервисам
4. **[Автоматическая инициализация](docs/13-TILT-INITIALIZATION.md)** - Что происходит при `make tilt-up`

### Разработка
- **[Переменные окружения](docs/03-ENVIRONMENT-VARS.md)** - Конфигурация и доступ к инфраструктуре
- **[Руководство по Tiltfile](docs/04-TILTFILE-GUIDE.md)** - Настройка Tilt для ваших сервисов
- **[Руководство по сервисам](docs/05-SERVICES-GUIDE.md)** - Создание и разработка микросервисов

### База данных
- **[Примеры запросов к БД](docs/09-DATABASE-QUERIES.md)** - Event sourcing, multi-tenancy, idempotency
- **[Materialized Views (deprecated)](docs/10-MATERIALIZED-VIEWS.md)** - Старая архитектура
- **[Инкрементальные Views](docs/15-INCREMENTAL-VIEW-UPDATES.md)** - Новая архитектура (читать!)
- **[Idempotency Refactoring](docs/14-IDEMPOTENCY-REFACTORING.md)** - История изменений

### Event-Driven Architecture
- **[Работа с Avro схемами](docs/11-AVRO-SCHEMAS.md)** - Автоматическая регистрация схем, best practices

### Production
- **[Production Deployment](docs/06-PRODUCTION-DEPLOYMENT.md)** - CI/CD и продакшен деплой

### Архитектура
- **[Спецификации архитектуры](docs/07-ARCHITECTURE-SPECS.md)** - Event sourcing, Kafka, архитектура БД
- **[План инфраструктуры](docs/08-INFRA-PLAN.md)** - План настройки инфраструктуры
- **[Архитектурные решения](docs/12-ARCHITECTURE-DECISIONS.md)** - Обоснование технических решений

## Быстрый старт

### Для Arch Linux / Ubuntu / Debian
```bash
# 1. Установите k3d (рекомендуется)
# Arch:
yay -S k3d
# Ubuntu/Debian:
...
# 2. Создайте кластер
k3d cluster create local-dev --servers 1 --agents 1
...
**Подробная инструкция:** [docs/03-LINUX-SETUP.md](docs/03-LINUX-SETUP.md)

### Для macOS / других Linux
...
**Что происходит автоматически при `make tilt-up`:**
- ✅ Запуск всей инфраструктуры (PostgreSQL, Redis, Kafka, Schema Registry)
- ✅ **Автоматическое применение миграций БД** (все V*.sql файлы)
- ✅ **Автоматическая регистрация Avro схем** (из `infra/schemas/`)
- ✅ Запуск всех микросервисов из `packages/`

**Порядок инициализации:**
1. Kubernetes namespace создаётся
2. Инфраструктура разворачивается (PostgreSQL, Redis, Kafka, etc.)
3. 🗄️ **Миграции применяются** (после готовности PostgreSQL)
4. 📋 **Схемы регистрируются** (после готовности Schema Registry)
5. 🚀 Микросервисы запускаются
