...
## 1. Materialized Views: Триггеры vs pg_cron

### ✅ Выбрано: PostgreSQL Triggers

**Обоснование:**
- Образ `citusdata/citus:12.1` не содержит pg_cron extension
- Триггеры работают "из коробки"
- Для беттинга задержка ~100-300ms приемлема
- Real-time данные без дополнительных зависимостей

**Альтернативы рассмотрены:**
- ❌ **pg_cron**: требует custom образ + сложность поддержки
- ❌ **Kubernetes CronJob**: задержка 1-5 минут, не real-time
- ❌ **External scheduler**: дополнительный сервис, сложность

**Компромиссы:**
- ➕ Real-time обновление
- ➕ Простота эксплуатации
- ➖ Добавляет ~150ms к INSERT операциям
- ➖ Может быть overhead на очень высокой нагрузке

**Когда пересмотреть:**
- Если INSERT операции становятся критично медленными (>500ms)
- Если нагрузка превышает 1000 req/sec
- Если требуется батчевая обработка событий
